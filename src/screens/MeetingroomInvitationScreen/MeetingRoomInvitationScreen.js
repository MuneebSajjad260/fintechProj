...
import { setTeamMembersMeetingRoom, setOtherInviteeMeetingRoom } from '../../shared/redux/slices/memberSelectionSlice';
...
const InvitationScreen = ({route}) => {
  ...
  const { FromTime, ToTime,repeatBooking ,  currentMeetingRoom , allocation,  isoStartTime , isoEndTime,recurringDaysData,dayPass, currentMeetingName, selectedDate,rescheduleTeamMembers,idReschedule,isRescheduleRequest,participant, Description} = route.params;
  console.log('testing--',FromTime,'-',ToTime,'-',repeatBooking,'-',recurringDaysData,'-',dayPass,'currentMeetingRoom----',currentMeetingRoom);
  console.log('date iso ----',isoStartTime,'----',isoEndTime,'---','--',allocation);
  console.log('rescheduleTeamMembers---22--',rescheduleTeamMembers);
  console.log('dayPass22--',dayPass,'participant--',participant);

  const isDarkMode = useSelector(state => state.mode.colorScheme);
  const userData=useSelector(selectUserData);
  const GetMembersLoading =useSelector(state=>state?.getMembers?.loading);
  console.log('email user data ---',userData?.Email,'---',userData?.IsTeamAdministrator);
  // console.log('isReschedule---',idReschedule,isRescheduleRequest);
  const navigation = useNavigation();
  const dispatch = useDispatch();

  const [seqId, setSeqId] = useState(rescheduleTeamMembers ? rescheduleTeamMembers.length  : 0);
  ...
  const [inviteeName, setInviteeName] = useState(rescheduleTeamMembers? rescheduleTeamMembers1 : [] );
  const [inviteeNameScreen, setInviteeNameScreen] = useState( rescheduleTeamMembers ?  rescheduleTeamMembers1 : [] );
  ...
  const onselectInvitee=(index,item)=>{
    ...
    if ((selectedMembers.length + inviteeName.length) < 7) {
      const i = inviteeName?.findIndex(index =>
  ...
  const handleSaveBtn = () => {
    ...
    console.log('rescheduleTeamMembers lemgth---',rescheduleTeamMembers?.length );
    setSeqId(seqId + 1);
    ...
    else
    {
      setUserName({ value: ' ' });
      setOtherInvitee(false);
      setInviteeName([...inviteeName, { Id:seqId, VisitorFullName:userName.value.replace(/^\s+/, ''), VisitorEmail:email.value.replace(/^\s+/, ''), IsTeamAdministrator:false, isPayingMember:false ,TypeName:'bookingVisitor'}]);
      setInviteeNameScreen([...inviteeNameScreen, { Id:seqId, VisitorFullName:userName.value.replace(/^\s+/, ''), VisitorEmail:email.value.replace(/^\s+/, ''), IsTeamAdministrator:false, isPayingMember:false ,TypeName:'bookingVisitor'}]);
      return;
  ...
  useEffect(() => {
    ...
    dispatch(GetMembers(userData?.TeamIds)).unwrap().then((result) => {
      // check result 

      console.log('----res plan result 667788a---', result);


      const fromIndex = result?.findIndex(item => item?.IsTeamAdministrator === true);
      const toIndex = 0;
      ...
      const filteredData = temp.filter(member => member.Email !== userData?.Email);
      ...
      const nonmatching= dayPass === true ? rescheduleTeamMembers ? rescheduleTeamMembers1 : [] : nonMatchingEmails ? nonMatchingEmails : [];
      setSeqId(seqId + 1); 
      ...
      const replaceId = matchingEmails.map(obj => {
        const teamMember = temp.find(member => member.Email === obj.VisitorEmail);
        if (teamMember) {
      ...
      >
        <View style={styles.innerContainer}>
          ...
          <View style={styles.allignInRow}>
            ...
            <View>
              <Txt style={styles.meetingInvitationText}>{Strings.meetingInvitation}</Txt>
              <Txt style={[styles.max7Text,{color:isDarkMode ? 'rgba(255, 255, 255, 0.5)' : 'rgba(0, 0, 0, 0.5)' }]}>{Strings.max7}</Txt>
            </View>
          </View>
          <View style={styles.InputTeamMemberContainer}>
            { !dayPass ?
              <View>
                <Txt style={styles.selectTeamMemberText}>{Strings.selectFromTeamMember}</Txt>
                <ShimmerPlaceHolder
                ...
                </ShimmerPlaceHolder>
                {
                  ...
                  teamMembersData?.map((item, index) => (
                    ...
                    <InputTeamMember
                      // resheduleMember={resheduleMember}
                      IsAdmin={userData?.IsTeamAdministrator ? item?.IsTeamAdministrator : false}
                      // IsPayingMember={isSelectPayingMember(item?.Id)}

                      isSelected={  isSelectedMember(item?.Id) ? true : false}

                      onPress={() => onSelectTeamMember(index, { VisitorFullName: item?.FullName,VisitorEmail:item?.Email,TypeName: 'bookingVisitor'  ,isAdministrator: item?.IsTeamAdministrator, Id: item?.Id, isPayingMember: isSelectPayingMember(item?.Id)})}
                ...
                <Txt style={styles.otherInvitesText}>{Strings.otherInvites}</Txt>
              </View>
              :
              null
            }
              ...
              inviteeNameScreen.map((item, index) => {
                ...
                return <>
                  ...
                  <InputViewOtherInvitee
                    ...
                    onPress={() => onselectInvitee(index, {VisitorFullName: item?.VisitorFullName,VisitorEmail:item?.VisitorEmail,TypeName: item?.TypeName, isAdministrator: item?.IsTeamAdministrator, Id: item?.Id, isPayingMember: isSelectPayingMember(item?.Id)})}
            ...
            {otherInvitee === false ?
              <PrimaryButton
                ...
                loading={false}
                title={'Add invitee'}
                disabled={((selectedMembers.length + inviteeName.length) < 7) ? false : true}
                small={false}
                onPress={() => {
                ...
                stylesContainer={[styles.addInviteeBtn,
                  { backgroundColor :isDarkMode ? AppTheme.COLORS.btnActiveDarkMode : AppTheme.COLORS.black,
                    borderWidth:isDarkMode ? 1: 0, borderColor:isDarkMode ? AppTheme.COLORS.white : null
                  }]}
              ...
              <View style={styles.inputFieldContainer}>
                ...
                <Input
                  ...
                  InputStyling={[styles.textInputContainer,{backgroundColor: isDarkMode ? AppTheme.COLORS.wrapperDarkModeBg : '#EEEEEE'}]}
                ...
                <View style={styles.allignInRow}>
                  <SecondaryButton
                    ...
                    styleMainContainer={[styles.cancelBtn,
                    ...
                    styleBtnTxt={[styles.canceText,{color:isDarkMode ? AppTheme.COLORS.white : AppTheme.COLORS.black ,}]}
                  />
                  <PrimaryButton
                    accessibilityLabel='SaveBtn'
                    loading={false}
                    title={'Save'}
                    disabled={(userName.value != '' && email.value != '' && userName.value != ' ' && email.value != ' ' && !email.error) ? false : true}
                    small={false}
                    onPress={() => {
                      handleSaveBtn();
                    }}
                    stylesContainer={[styles.saveBtn, { backgroundColor :isDarkMode ? AppTheme.COLORS.btnActiveDarkMode : AppTheme.COLORS.black,
                      borderWidth:isDarkMode ? 1: 0, borderColor:isDarkMode ? AppTheme.COLORS.white : null}]}
        ...
        <View style={styles.proceedBtnContainer}>
          <Botton
            ...
            title={(selectedMembers.length === 0 && inviteeName.length === 0) ? 'Skip' : 'Confirm'}
            disabled={false}
            small={false}
            onPress={() => {
              navigation.navigate(ScreensName.Meetingsummary,{
                ...
                recurringDaysData:recurringDaysData,
                dayPass:dayPass,
              ...
              const newSelectedMember=selectedMembers.map(({VisitorEmail,VisitorFullName,TypeName})=>
              ...
              const newSelectedInviteeData=inviteeName.map(({VisitorEmail,VisitorFullName,TypeName})=>
                ({VisitorFullName,VisitorEmail,TypeName}));
              dispatch(setOtherInviteeMeetingRoom(newSelectedInviteeData));
