...
import {
  selectTeamMembersMeetingRoom,
  selectOtherInviteeMeetingRoom,
...
import { convertMinToHrAndMin } from '../../shared/utils/helper';
...
const MeetingSummaryScreen = ({route}) => {
  ...
  const meetingReqLoading = useSelector(
    state => state?.meetingRoomRequest?.loading,
  );
  const meetingRescheduleLoading = useSelector(
    state => state?.meetingRoomReschedule?.loading,
  );
  const ParticipantRescheduleLoading = useSelector(
    state => state?.participantReschedule?.loading,
  );
  MeetingRoomRequest;
  ...
  //   duration
  // );
  // console.warn('RecurringDaysData:--------------', recurringDaysData.id, repeatBooking);
  // console.log(
  //   ' currentMeetingName---',
  //   currentMeetingName,
  //   'participant--',
  ...
  const meetingRoomCard = {
    id: 1,
    ResourceName: currentMeetingName,
    BookingVisitors: selectedMembers.concat(selectedInviteeData),
    desc: 'A room has a big table in the middle with Smart TV',
    today: 'Today',
    date: start_end_time,
    duration: 'Duration',
    durationTime: convertMinToHrAndMin(duration),
  };
  const inviteesCard = {
    id: 1,
    invitees: 'Invitees',
    teamMembers: selectedMembers.concat(selectedInviteeData),
    otherInvitee: selectedInviteeData,
  };
  ...
  const onSubmit = data => {
    console.warn('Repair The Passed Data:------------', data);
    // console.warn('Repair The Passed Data:------------', typeof data.ResourceId, typeof data.CoworkerId, repeatBooking);
    dispatch(MeetingRoomRequest(data))
      .unwrap()
      .then(result => {
        ...
        if (result?.statusCode === 200) {
          ...
          if (userData?.TeamIds) {
            ...
            if (dayPass && priceDayPass == 0) {
              navigation.navigate(ScreensName.bookingConfirmed, {
                dayPass: true,
              });
            } else {
              if (repeatBooking === false || !recurringDaysData) {
                navigation.navigate(ScreensName?.dayPassPayment, {
                  dueDate: moment(now).format('Do MMMM, YYYY'),
                  price: data?.Price,
                  id: result?.data?.id,
                  ...
                  resheduleData: {
                    ...
                    Price: dayPass
                      ? priceDayPass
                      : convertTimeToDecimal(`${start_end_time.split(' ')[0]+start_end_time.split(' ')[3]}`, `${start_end_time.split(' ')[2]+start_end_time.split(' ')[3]}`)
                      * payment?.Price,
                ...
                navigation.navigate(ScreensName?.dayPassPayment, {
                  ...
                  resheduleData: {
                    ...
                    isRepeatBooking: repeatBooking,
                    isTeamBooking: true,
                    RepeatEvery: recurringDaysData.id === 2 ? 2 : 1,
                    Repeats:
                      recurringDaysData.id === 0
                        ? '1'
                        : recurringDaysData.id === 3
                        ? '3'
                        : '2',
                    ...
                    Price: dayPass
                      ? priceDayPass
                      : convertTimeToDecimal(`${start_end_time.split(' ')[0]+start_end_time.split(' ')[3]}`, `${start_end_time.split(' ')[2]+start_end_time.split(' ')[3]}`) * payment?.Price,
                    isRescheduleRequest: isRescheduleRequest,
                    rescheduleId: idReschedule,
                  },
                });
          ...
          console.warn('testing data-----', data);
          console.warn('result reject --', result);
          showDatePicker();
        }
      })
      .catch(err => {
  ...
  const onSubmitReschedule = data => {
    ...
    dispatch(MeetingRoomRequest(bookingData))
      .unwrap()
      .then(result => {
        // console.log('result for booking in reshedule---', result);
        if (result?.statusCode === 200) {
          dispatch(MeetingRoomReschedule(data))
            .unwrap()
            .then(result => {
              dispatch(setMeetingRoomid(result?.data?.id));
  ...
  const participantReschedule = data => {
    ...
    dispatch(ParticipantReschedule(data))
      .unwrap()
      .then(result => {
        dispatch(setMeetingRoomid(result?.data?.id));
        if (result?.statusCode === 200) {
  ...
  useEffect(() => {
    dispatch(MeetingRoomPrice(currentMeetingRoom))
      .unwrap()
      .then(result => {
        console.warn('result---', result.resource);
  ...
  useEffect(() => {
    dispatch(BookingSetting())
      .unwrap()
      .then(result => {
        console.log('booking setting result ----', result?.setting);
        setTentative(
          dayPass
            ? result?.setting?.passUserMeetingRoomConfirmation
            : result?.setting?.memberMeetingRoomConfirmation,
        );
      })
      .catch(err => {
  ...
  const dateCalculate = date => {
    ...
    const formattednewDate = moment(newdate, 'DD/MM/YYYY').format(
      'DD, MMM YYYY',
    );
  ...
  return (
    <Frame screenTitle="Meeting Summary">
      <ScrollView
          ...
          <View style={styles.subContainer}>
            ...
            <View
              ...
              <View
                ...
                {dayPass ? (
                  <View>
                    ...
                    <Txt
                      accessibilityLabel="dayPassPrice"
                      style={styles.paymentContainerValues}>
                      {DayPassPriceLoading === false
                        ? `SAR ${priceDayPass}`
                        : null}
                    </Txt>
                  </View>
                  ...
                  <View>
                    <ShimmerPlaceHolder
                      ...
                      shimmerStyle={styles.shimmerPlanFee}></ShimmerPlaceHolder>
                    <Txt
                      accessibilityLabel="meetingroomprice"
                      style={styles.paymentContainerValues}>
                      {priceLoading === false ? `SAR ${payment?.Price}` : null}
                    </Txt>
                  </View>
                )}
              </View>
                ...
                {dayPass ? (
                  <View>
                    <ShimmerPlaceHolder
                      visible={DayPassPriceLoading === false}
                      shimmerStyle={[
                        styles.shimmerPlanFee,
                        {marginLeft: normalize(164)},
                      ]}></ShimmerPlaceHolder>
                    <Txt
                      accessibilityLabel="daypasshours"
                      style={styles.paymentContainerValues}>
                      {DayPassPriceLoading === false
                  ...
                  <View>
                    ...
                    <Txt
                      accessibilityLabel="meetingroomhours"
                      style={styles.paymentContainerValues}>
                      {priceLoading === false
                        ? convertMinToHrAndMin(duration)
                        : null}
                ...
                {dayPass ? (
                  <View>
                    <ShimmerPlaceHolder
                      ...
                      shimmerStyle={[
                        styles.shimmerPlanFee,
                        {marginLeft: normalize(200), width: '18%'},
                      ]}></ShimmerPlaceHolder>
                    <Txt
                      accessibilityLabel="daypassSubtotal"
                      style={styles.paymentContainerValues}>
                      {DayPassPriceLoading === false ? priceDayPass : null}
                    </Txt>
                  </View>
                ) : (
                  <View>
                    ...
                    <Txt
                      accessibilityLabel="meetingroomSubtotal"
                      style={styles.paymentContainerValues}>
                      {priceLoading === false
                        ? convertTimeToDecimal(`${start_end_time.split(' ')[0]+start_end_time.split(' ')[3]}`, `${start_end_time.split(' ')[2]+start_end_time.split(' ')[3]}`) *
                          payment?.Price
              ...
              <View
                ...
                <Txt style={styles.totalPayableText}>
                ...
                {dayPass ? (
                  <View>
                    ...
                    <Txt
                      accessibilityLabel="daypassTotal"
                      style={[
                  ...
                  <View>
                    ...
                    <Txt
                      accessibilityLabel="meetingRoomTotal"
                      style={[
                        styles.totalPayableValue,
                        {
                          color: isDarkMode
        ...
        <View>
          <Botton
            loading={
              meetingReqLoading ||
              meetingRescheduleLoading ||
              ParticipantRescheduleLoading
                ? true
                : false
            }
            ...
            <View style={styles.mainModalContainer}>
              <View style={styles.innerModal}>
                ...
                <Txt style={styles.desc}>
                  This booking cannot be created at this time, please try again
                  later
                </Txt>
              </View>
            </View>
