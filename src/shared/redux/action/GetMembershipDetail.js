...
export const GetMembershipDetail = createAsyncThunk(
  'getMembershipDetail/GetMembershipDetail',
  async (id, thunkAPI) => {
    const endpointName = '(Membership Details)';

    try {
      if (!Number.isInteger(id)) {
        throw new Error('Please provide a valid integer Team ID.');
      }
      ...
      const response = await instance.get(
        `${API_ENDPOINTS.fintech_backend_url}/membership-details/${id}`,
      );
